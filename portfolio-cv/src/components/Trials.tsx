...
const ROW1 = [TRIALS_DATA[0], TRIALS_DATA[1], TRIALS_DATA[2], TRIALS_DATA[3], TRIALS_DATA[4]];
const ROW2 = [TRIALS_DATA[5], TRIALS_DATA[6], TRIALS_DATA[0], TRIALS_DATA[1], TRIALS_DATA[2]];
const ROW3 = [TRIALS_DATA[3], TRIALS_DATA[4], TRIALS_DATA[5], TRIALS_DATA[6], TRIALS_DATA[0]];
...
export default function Trials() {
  return (
    <div className="w-full relative py-20 lg:py-32 overflow-hidden flex flex-col justify-center min-h-[100dvh]">
      ...
      {/* ── Heading ──────────────────────────────────────────────────────── */}
      <div className="w-full max-w-3xl mx-auto text-center px-4" style={{ paddingBottom: '3.5rem' }}>
        <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
          className="text-[11px] font-black tracking-[0.25em] uppercase mb-3 flex items-center justify-center gap-3" style={{ color: 'var(--primary)' }}>
          <span className="w-8 h-px bg-current opacity-40"></span>
          Section IV · บททดสอบที่ก้าวผ่าน
          <span className="w-8 h-px bg-current opacity-40"></span>
        </motion.p>
        
        <motion.h2 initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.05, duration: 0.5 }}
          className="text-5xl sm:text-6xl font-black tracking-tighter uppercase mb-4" style={{ color: 'var(--fg-main)' }}>
        ...
        <motion.p initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.15 }}
          className="text-sm sm:text-base max-w-lg mx-auto leading-relaxed" style={{ color: 'var(--fg-main)', opacity: 0.55 }}>
      ...
      >
        {MARQUEE_ROWS.map((row) => (
          <div key={row.id} className="animate-marquee items-center gap-6 px-3"
            style={{ 
              animationDuration: row.speed,
              animationDirection: row.direction as any
            }}>
            {row.items.map((item, idx) => (
              <div key={`${row.id}-${item.id}-${idx}`} 
                ...
                <div className="absolute top-0 left-0 w-full h-1 opacity-40 group-hover:opacity-100 transition-opacity" 
                ...
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wider mb-1" style={{ color: item.color }}>
                    {item.category}
                  </p>
                  <h3 className="text-xl font-black leading-tight mb-2" style={{ color: 'var(--fg-main)' }}>
                    {item.title}
                  </h3>
                  <p className="text-xs font-medium" style={{ color: 'var(--fg-main)', opacity: 0.55 }}>
                    {item.detail}
                  </p>
                </div>
              </div>
